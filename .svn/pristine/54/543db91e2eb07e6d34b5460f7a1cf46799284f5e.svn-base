  ...
  <div class="app-container outhound">
    ...
    >
      ...
      <el-form-item label="街道名称" prop="tempRegion">
        <el-select
          ...
          placeholder="请选择街道名称"
          clearable
          @change="one_tempchange(queryParams.tempRegion)"
        >
          <el-option
      ...
      <el-form-item label="任务名称" prop="tempTaskid">
        <el-select
          v-model="queryParams.tempTaskid"
          placeholder="请选择任务名称"
          clearable
        >
          <el-option
            v-for="dict in rw_list_one"
            :key="dict.value"
            :label="dict.value"
            :value="dict.id"
          />
    ...
    >
      ...
      <el-table-column
        ...
        prop="tempTaskName"
      ></el-table-column>
      <el-table-column label="标签名称" align="center" prop="tempLabelName" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="900px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item
          label="任务标签"
          v-show="rw_list.length > 0 && tempLabellist.length > 0"
        >
          <el-checkbox-group v-model="form.tempLabel">
            <el-checkbox
              v-for="(dict,i) in tempLabellist"
              :label="dict.id"
              :key="i"
...
export default {
  ...
  methods: {
    ...
    one_tempchange(id) {
      this.rw_list_one = [];
      this.queryParams.tempTaskid = undefined;
      let data = {
        regionId: id,
      };
      getrwlist(data).then((res) => {
        this.rw_list_one = res.data;
      });
    },
    // 重置新增
    clear_fromdata() {
